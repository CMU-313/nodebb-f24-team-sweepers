...
describe("Topic's", () => {
	...
	describe('order pinned topics', () => {
		...
		it('should order pinned topics with latest pinned on top', async () => {
			...
			const pinnedTopics = await db.getSortedSetRange(
				`cid:${categoryObj.cid}:tids:pinned`,
				0,
				-1
			);
			await Promise.all(
				pinnedTopics.map((tid) => topics.tools.unpin(tid, adminUid))
			);
			// Now pin the topic and check if it's on top
			await topics.tools.pin(tid3, adminUid);
			const updatedPinnedTopics = await db.getSortedSetRange(
				`cid:${categoryObj.cid}:tids:pinned`,
				0,
				-1
			);
